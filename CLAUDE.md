...
- **Painel do quiosque em tempo real** (`/painel/[eventoId]/...`, senha única em `PAINEL_QUIOSQUE_SENHA`, fila atualizada por polling a cada 3s — ver "Por que polling" abaixo). Dois fluxos de status distintos para o mesmo `SubPedido`, conforme a modalidade do quiosque:
  ...
  - **Produtos**: página própria (`/painel/[eventoId]/q/[quiosqueId]/produtos`) onde o **próprio quiosque** cria, edita (nome/preço/tempo-ou-duração/estoque) e esgota/reativa (`ativo`) seus produtos — decisão de negócio: quem cadastra produto é o responsável pelo quiosque, não o gestor do evento.
