...
export function ExcluirEventoButton({ eventoId, eventoNome }: { eventoId: string; eventoNome: string }) {
  ...
  async function excluir() {
    ...
    setExcluindo(true);
    try {
      const resposta = await fetch(`/api/eventos/${eventoId}`, { method: "DELETE" });
      const dados = await resposta.json().catch(() => ({}));
      if (!resposta.ok) {
        setErro(dados.erro ?? "Não foi possível excluir o evento.");
        setExcluindo(false);
        return;
      }
      router.push("/gestor");
  ...
  return (
    <div className="cartao" style={{ borderColor: "#FFD9C8" }}>
      ...
      <p className="texto-fraco" style={{ marginBottom: 10 }}>
        Isso apaga o evento, todos os quiosques e produtos dele. Só é possível se ainda não houver
        nenhum pedido pago registrado — essa ação não pode ser desfeita.
      </p>
      <div className="campo">
        <label>Digite "{eventoNome}" pra confirmar</label>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button
          ...
          disabled={confirmacao !== eventoNome || excluindo}
          onClick={excluir}
        >
